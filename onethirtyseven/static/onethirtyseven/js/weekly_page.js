...
function drawMovieGraph() {
    ...
    var weekly_gross_line = d3.svg.line() // added the total gross line
        .x(function(d) { return x(d.year); })
        .y(function(d) { return y2(d.total_gross); });

    var svg = d3.select("#weekly_graph").append("svg")
    ...
    dataset.forEach(function(d) {
      ...
      d.infl_adj_total = +d.infl_adj_total;
    });

    dataset.

    x.domain(d3.extent(dataset, function(d) { return d.year; }));
    y.domain([0, d3.max(dataset, function(d) { return d.total_gross; })]);
    y2.domain([0, d3.max(dataset, function(d) { return d.infl_adj_total; })]);
    ...
    svg.selectAll(".bar")
      ...
      .enter().append("rect")
        ...
        .attr("width", (width / d3.max(dataset, function(d) { return d.year })) - barPadding)
        .attr("y", function(d) { return y(d.total_gross); })
        .attr("height", function(d) { return height - y(d.total_gross); })
        .style("opacity", 1)
        .on("mousemove", function (d) {
            d3.select("#tooltip")
                ...
                .style("opacity", 1)
                .select("#value")
                .text(d.year + " - " + dollar(d.total_gross));
        })
